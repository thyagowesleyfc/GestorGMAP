...
describe("receiving entry migration", () => {
  it("creates receiving entries and items without stock movement side effects", async () => {
      ...
      const tables = await client.query<{ table_name: string }>(
        `select table_name
          ...
          where table_schema = 'public'
            and table_name in ('receiving_entry', 'receiving_entry_item', 'stock_movement', 'stock_position')
          order by table_name`
      );
